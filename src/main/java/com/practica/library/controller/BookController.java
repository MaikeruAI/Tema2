...
//        return bookService.getBooks();
    ...
    public List<BookDTO> getReadBooks() {
        return bookServiceJPA.getReadBooks();
    }
...
//        bookService.addBook(mapper.convertValue(book, Book.class));
    ...
    public ResponseEntity<String> markAsRead(@RequestParam Long id) {
        boolean updated = bookServiceJPA.markBookAsRead(id);
        if (updated) {
            return ResponseEntity.ok("✅ Cartea cu ID " + id + " a fost marcată ca citită în DB!");
        } else {
